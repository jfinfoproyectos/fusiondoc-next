...
export function ThemeSelector({ themes }: ThemeSelectorProps) {
  ...
  const handleFontLoading = (css: string) => {
    ...
    fontVars.forEach(v => {
      ...
      if (match && match[1]) {
        // Tomar la primera fuente de la lista (ej. 'Source Code Pro')
  ...
  // Si no está montado, mostramos un botón ghost estático para evitar un flash of unmatched content
  if (!mounted) {
    return (
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-[150px]">
        ...
        >
          ...
          {activeTheme === "default" && <Check className="w-4 h-4 ml-2" />}
        </DropdownMenuItem>
        ...
        {themes.map((theme) => (
          ...
          >
            ...
            {activeTheme === theme.id && <Check className="w-4 h-4 ml-2" />}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
